...
class NToggleSwitch(
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.NToggleSwitch, 0, 0).apply {
                try {
                    maxLevel = getInteger(R.styleable.NToggleSwitch_level, 2)
                    val text1 = getString(R.styleable.NToggleSwitch_text0) ?: "Option0"
                    val text2 = getString(R.styleable.NToggleSwitch_text1) ?: "Option1"
                    val text3 = getString(R.styleable.NToggleSwitch_text2) ?: "Option2"
                    options = listOf(text1, text2, text3)
                } finally {
                    recycle()
                }
        }
        when (maxLevel) {
            2 -> { inflater.inflate(R.layout.toggle_switch_level2, this, true) }
            3 -> { inflater.inflate(R.layout.toggle_switch_level3, this, true) }
            else -> {}
        }
